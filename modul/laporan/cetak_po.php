    ...
    class PDF extends FPDF{
	   function FancyTable(){
        ...
        while($data=mysql_fetch_array($sql)){
			...
			if ($data['kode_barang']=='HPAXC5'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
			if ($data['kode_barang']=='HPACK5'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
			if ($data['kode_barang']=='HPCDK5'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
			if ($data['kode_barang']=='HPCTXI'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
			if ($data['kode_barang']=='HPHM5K'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
			if ($data['kode_barang']=='HPLAXI'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
			if ($data['kode_barang']=='HPLM15'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
			if ($data['kode_barang']=='HPLTXI'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
			if ($data['kode_barang']=='HPRTD1'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
			if ($data['kode_barang']=='HPSTSI'){ $hargabeliproduk = hargatabprod($data['kode_barang']);}
		  ...
		  $this->Cell($w[5],6,$hargabeliproduk.$popusatket,1,0,'L',$fill);
          $this->Ln();  
		  $no++;
        }
...
//	$tgl = date('d M Y');
	...
	if(user_buat_po($kode)=='LESTARIKPS'){$pdf->Image('images/ttd/ari3.png',145,$pdf->GetY()-4,40);}
	if(user_buat_po($kode)=='ASTRIKPS'){$pdf->Image('images/ttd/Astri3.png',145,$pdf->GetY()-4,40);}
	if(user_buat_po($kode)=='AGUSKPS'){$pdf->Image('images/ttd/agus3.png',145,$pdf->GetY()-4,40);}
