...
//echo '--1--'."</br>";	
   		if(empty($kode)){
			...
			$query = "SELECT * FROM konfirm_retur_beli_pre";
			echo "<h1>Maaf, data tidak ditemukan</h1><br>";
		}else{
			$query 	= "SELECT * FROM konfirm_retur_beli_pre WHERE kodekonf_retur='$kode'";
				}	 
//echo '--2--'.$query."</br>";	
...
//echo '--5--'.$query."</br>";
	...
	function Footer()
	{
	   ...
	   function FancyTable(){
			...
			$kode	= $_GET[kode];   		
				if(empty($kode)){
							$query = "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan, b.`satuan`,
												d.batch,d.ed,d.alasan_retur,d.asal_retur,d.do_pusat,d.tgl_do_pusat
												FROM konfirm_retur_beli_pre a 
												LEFT JOIN retur_beli_pre d ON a.`kode_retur`=d.`kode_retur` AND a.`kode_barang`=d.`kode_barang` AND a.batch=d.batch
												LEFT JOIN mstproduk b ON a.`kode_barang`=b.`kodeproduk` 
												LEFT JOIN mastercabang c ON a.`cabang`=c.`KdCabang`";
					}else{
							$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan, b.`satuan`,
												d.batch,d.ed,d.alasan_retur,d.asal_retur,d.do_pusat,d.tgl_do_pusat,
												cc.`alasan`,dd.`asal_retur_ket`
												FROM konfirm_retur_beli_pre a 
												LEFT JOIN retur_beli_pre d ON a.`kode_retur`=d.`kode_retur` AND a.`kode_barang`=d.`kode_barang` AND a.batch=d.batch
												LEFT JOIN mstproduk b ON a.`kode_barang`=b.`kodeproduk` 
												LEFT JOIN mastercabang c ON a.`cabang`=c.`KdCabang`
												LEFT JOIN mst_alasan_retur cc ON d.`alasan_retur`=cc.`kode_alasan`
												LEFT JOIN `mst_asal_retur` dd ON d.`asal_retur`=dd.`kode_asal`
												WHERE a.kodekonf_retur='$kode' AND jumlah_retur_valid<>0";
						} 
						...
						while($data=mysql_fetch_array($sql)){
						  ...
						  $this->Cell($w[6],5,$data['jumlah_retur_valid'],1,0,'C',$fill);
...
//	$tgl = date('d M Y');
    ...
    $pdf->Cell(0,5,'FORM PELAPORAN PENGEMBALIAN BARANG RUSAK DAN ED KE SUPPLIER (PRECURSOR)',0,1,'C');
    ...
    $pdf->Cell(75,5,': '.kode_retur_konfirm($kode),0,0,'L');
    $pdf->Cell(50,5,'No. Validation ',0,0,'L');
    $pdf->Cell(75,5,': '.$kode,0,1,'L');
...
//	$pdf->SetX(2);	
	...
	$pdf->Cell(70,3,'',0,0,'C',false);
	$pdf->Cell(70,3,"Total Koli : ".koli_retur_cetak($kode)." Koli",0,1,'R',false);
	...
	//$pdf->Cell(0,3,tanggal_retur_edit($kode),0,1,'',false);
	$pdf->Ln(2);
