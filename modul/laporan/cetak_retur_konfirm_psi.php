...
//echo '--1--'."</br>";	
   		if(empty($kode)){
			...
			$query = "SELECT * FROM konfirm_retur_beli_psi";
			echo "<h1>Maaf, data tidak ditemukan</h1><br>";
		}else{
			$query 	= "SELECT * FROM konfirm_retur_beli_psi WHERE kodekonf_retur='$kode'";
				}	 
//echo '--2--'.$query."</br>";	
			...
			$kode	= $_GET[kode];   		
				if(empty($kode)){
							$query = "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan, b.`satuan`,
												...
												FROM konfirm_retur_beli_psi a 
												LEFT JOIN retur_beli_psi d ON a.`kode_retur`=d.`kode_retur` AND a.`kode_barang`=d.`kode_barang` AND a.batch=d.batch
