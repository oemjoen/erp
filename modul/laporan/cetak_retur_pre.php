...
//echo '--1--'."</br>";	
   		if(empty($kode)){
			...
			$query = "SELECT * FROM retur_beli_pre";
			echo "<h1>Maaf, data tidak ditemukan</h1><br>";
		}else{
			$query 	= "SELECT * FROM retur_beli_pre WHERE kode_retur='$kode'";
				}	 
//echo '--2--'.$query."</br>";	
...
//echo '--5--'.$query."</br>";
	...
	function Footer()
	{
	   ...
	   function FancyTable(){
			...
			$kode	= $_GET[kode];   		
				if(empty($kode)){
							$query = "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan FROM retur_beli_pre a 
							...
							$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan,
												cc.`alasan`,dd.`asal_retur_ket` FROM retur_beli_pre a 
												LEFT JOIN mstproduk b ON a.`kode_barang`=b.`kodeproduk` 
												LEFT JOIN mastercabang c ON a.`cabang`=c.`KdCabang` 
