  ...
  date_default_timezone_set('Asia/Jakarta'); 
  	...
  	$kode	= $_GET[kode];
   		...
   		if(empty($kode)){
			...
			$query = "SELECT * FROM po_pembelian_pre WHERE relokasi_jumlah_beli_valid1<>0";
		}else{
			$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang,d.`ket_prinsipal`,b.satuan FROM po_pembelian_pre a 
							LEFT JOIN mstproduk b ON a.`kode_barang`=b.`kodeproduk` 
							LEFT JOIN mastercabang c ON a.`cabang`=c.`KdCabang` 
							LEFT JOIN pembelian_pre d ON a.`kode_beli`=d.`kode_beli` AND a.`kode_barang`=d.`kode_barang`
   ...
   if ($row>0) {
    ...
    class PDF extends FPDF{
	   function FancyTable(){
		...
		$kode	= $_GET[kode];   		
		if(empty($kode)){
			$query = "SELECT a.*,b.`satuan` FROM po_pembelian_pre a
						LEFT JOIN `mstproduk` b ON a.`kode_barang`=b.`kodeproduk` 
...
//	$pdf->Cell(60,5,$namacabang,0,1,'L');
    $pdf->SetFont('arial','B',12);
    $pdf->Cell(0,5,'SURAT PESANAN RELOKASI BARANG PREKURSOR',0,1,'C');
	$pdf->SetX(10);
    $pdf->SetFont('arial','B',12);
