  ...
  date_default_timezone_set('Asia/Jakarta'); 
  	...
  	$kode	= $_GET[kode];
   		...
   		if(empty($kode)){
			...
			$query = "SELECT * FROM po_pembelian WHERE relokasi_jumlah_beli_valid<>0";
		}else{
			$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang,d.`ket_prinsipal`,b.satuan FROM po_pembelian a 
						...
						WHERE a.kodepo_beli='$kode' AND a.relokasi_jumlah_beli_valid<>0";
			}	 
	
  	$sql 	= mysql_query($query);
	$row	= mysql_num_rows($sql);

	$namacabang = cari_nama_cabang_relokasi($kode);
	   ...
	   function FancyTable(){
		...
		if(empty($kode)){
			$query = "SELECT a.*,b.`satuan` FROM po_pembelian a
						LEFT JOIN `mstproduk` b ON a.`kode_barang`=b.`kodeproduk` 
					WHERE a.relokasi_jumlah_beli_valid<>0";
		}else{
			$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang,d.`ket_prinsipal`,b.satuan FROM po_pembelian a 
							LEFT JOIN mstproduk b ON a.`kode_barang`=b.`kodeproduk` 
							LEFT JOIN mastercabang c ON a.`cabang`=c.`KdCabang` 
        ...
        while($data=mysql_fetch_array($sql)){
		  ...
		  $this->Cell($w[1],6,$data['relokasi_jumlah_beli_valid'],1,0,'C',$fill);
