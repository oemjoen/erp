...
//echo '--1--'."</br>";	
   		if(empty($kode)){
			...
			$query = "SELECT * FROM retur_beli";
			echo "<h1>Maaf, data tidak ditemukan</h1><br>";
		}else{
			$query 	= "SELECT * FROM retur_beli WHERE kode_retur='$kode'";
				}	 
//echo '--2--'.$query."</br>";	
...
//echo '--5--'.$query."</br>";
	...
	function Footer()
	{
	   ...
	   function FancyTable(){
			...
			$kode	= $_GET[kode];   		
				if(empty($kode)){
							$query = "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan FROM retur_beli a 
