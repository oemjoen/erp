  ...
  date_default_timezone_set('Asia/Jakarta'); 
  	...
  	$kode	= $_GET[kode];
   		...
   		if(empty($kode)){
			...
			$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang FROM pembelian_usulan_pre a 
	...
	function FancyTable2(){
			...
			$kode	= $_GET[kode];   		
				if(empty($kode)){
						...
						$no=1;
						while($data=mysql_fetch_array($sql)){
							$xxx = cek_status_moving($data['kode_barang'],$data['cabang']);
						  ...
						  if(!empty($xxx)){$this->Cell($w[3],5,cek_status_moving($data['kode_barang'],$data['cabang']),0,0,'L',$fill);}
						  $this->Ln();  
						  $no++;
						}
							$this->Cell(array_sum($w),0,'','T');
