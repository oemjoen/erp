...
//echo '--1--'."</br>";	
   		if(empty($kode)){
			...
			$query = "SELECT * FROM konfirm_retur_beli";
			echo "<h1>Maaf, data tidak ditemukan</h1><br>";
		}else{
			$query 	= "SELECT * FROM konfirm_retur_beli WHERE kodekonf_retur='$kode'";
				}	 
//echo '--2--'.$query."</br>";	
			...
			$kode	= $_GET[kode];   		
				if(empty($kode)){
							$query = "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan, b.`satuan`,
												...
												LEFT JOIN retur_beli d ON a.`kode_retur`=d.`kode_retur` AND a.`kode_barang`=d.`kode_barang` AND a.batch=d.batch
...
//	$tgl = date('d M Y');
    ...
    $pdf->Cell(0,5,'FORM PELAPORAN PENGEMBALIAN BARANG RUSAK DAN ED KE SUPPLIER',0,1,'C');
