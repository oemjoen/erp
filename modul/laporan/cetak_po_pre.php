  ...
  date_default_timezone_set('Asia/Jakarta'); 
  	...
  	$kode	= $_GET[kode];
   		...
   		if(empty($kode)){
			...
			$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang FROM po_pembelian_pre a 
	...
	$ijinpbf = cari_ijin_pbf_po($kode);
	$alamatpbf = cari_alamat_pbf_po($kode);
	$telppbf = cari_telp_pbf_po($kode);
	$faxpbf = cari_fax_pbf_po($kode);
	   ...
	   function FancyTable(){
			...
			$kode	= $_GET[kode];   		
				if(empty($kode)){
							...
							$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan,b.satuan FROM po_pembelian_pre a 
												LEFT JOIN mstproduk b ON a.`kode_barang`=b.`kodeproduk` 
												LEFT JOIN mastercabang c ON a.`cabang`=c.`KdCabang` 
												WHERE a.kodepo_beli='$kode' AND a.jumlah_beli_valid<>0";
						} 
...
//	$tgl = date('d M Y');
    ...
    $pdf->Cell(0,5,'SURAT PESANAN OBAT MENGANDUNG PRECURSOR FARMASI',0,1,'C');
    ...
    $pdf->Cell(30,5,': Penanggung Jawab PT. Sapta Sari Tama '.cari_nama_cabang_pusat($kode),0,1,'L');
    $pdf->Cell(30,5,'SIKA',0,0,'L');
    $pdf->Cell(30,5,': '.cari_sika_pusat($kode),0,1,'L');
	$pdf->Ln(2);
    $pdf->Cell(0,5,'Mengajukan pemesanan obat mengandung Prekursor Farmasi, kepada :',0,1,'L');
	...
	$pdf->Ln(2);
    $pdf->Cell(0,5,'Jenis obat yang mengandung Prekursor Farmasi yang dipesan adalah :',0,1,'L');
	...
	$pdf->Cell(30,5,'Zat Aktif Precursor',1,0,'C',true);
	...
	$pdf->Ln(2);
	$pdf->SetFont('arial','',10);
    $pdf->Cell(0,5,'Obat mengandung Precursor Farmasi tersebut akan digunakan untuk memenuhi kebutuhan :',0,1,'L');
	$pdf->Ln(2);
    $pdf->Cell(30,5,'Nama PBF',0,0,'L');
    $pdf->Cell(30,5,': PT. Sapta Sari Tama Cabang Pusat',0,1,'L');
    $pdf->Cell(30,5,'Alamat PBF',0,0,'L');
    $pdf->Cell(30,5,': '.cari_alamat_pbf_pusat($kode),0,1,'L');
    $pdf->Cell(30,5,'Nomor Izin PBF',0,0,'L');
    $pdf->Cell(30,5,': '.cari_ijin_pbf_pusat($kode),0,1,'L');
	
//    $pdf->Cell(30,5,': '.$cari_alamat_pbf_pusat($kode),0,1,'L');
//    $pdf->Cell(30,5,'Nomor Izin PBF',0,0,'L');
//   $pdf->Cell(30,5,': '.$cari_telp_pbf_pusat($kode),0,1,'L');
	
	
	$pdf->Ln(2);
	$pdf->SetFont('arial','',10);
    $pdf->Cell(0,5,'Atas Permintaan dari :',0,1,'L');
	$pdf->Ln(2);
    $pdf->Cell(30,5,'Nama PBF',0,0,'L');
    $pdf->Cell(30,5,': PT. Sapta Sari Tama Cabang '.cari_nama_cabang_po($kode),0,1,'L');
    $pdf->Cell(30,5,'Alamat PBF',0,0,'L');
