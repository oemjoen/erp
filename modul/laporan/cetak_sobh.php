  ...
  date_default_timezone_set('Asia/Jakarta'); 
	...
	$kodecabbtb 	= $_GET[btbcab];
   		...
   		if(empty($kode)){
			...
			$query 	= "SELECT * FROM trans_sobh a WHERE a.kode_sobh='$kode'";
			}	 
   ...
   if ($row>0) {
	...
	function Header() 
	{
		...
		$this->Cell(0,5,'STOK OPNAME BARANG HARIAN (SOBH) SUMMARY',0,1,'R');
		$this->$_GET[kode];
		$this->Ln(25);
		$this->SetFont('arial','B',12);
		$this->Cell(0,5,'LAPORAN HASIL STOK OPNAME BARANG HARIAN (SOBH)',0,1,'C');
		$this->Ln(5); 
		//$this->SetX(10);
	}
	...
	function selisihTable(){
		...
		if(empty($kode)){
			...
			$query 	= "SELECT * FROM (SELECT a.`kodecabang`,a.`kode_sobh`,a.`tgl_sobh`,b.namasupplier,a.`kode_produk`,
 	...
 	function sudahEdTable(){
		...
		if(empty($kode)){
			...
			$query 	= "SELECT * FROM (	
				(	
					SELECT * FROM (
					SELECT a.`kodecabang`,a.`kode_sobh`,a.`tgl_sobh`,b.namasupplier,a.`kode_produk`,
								b.namaproduk,COALESCE(a.`hpc`,0) AS hpc,b.satuan,
								COALESCE(a.qty_komp,0)  AS unitkomp, 
							...
							LEFT JOIN (SELECT a.`kodeproduk`,a.namaproduk,a.`kodesupplier`,b.`namasupplier`,a.satuan FROM `mstproduk` a 
										LEFT JOIN `mstsupplier2` b ON a.`kodesupplier`=b.`kodesupplier`) b ON a.`kode_produk`=b.kodeproduk 
							WHERE a.kode_sobh='$kode' AND a.ed_1 <> '0000-00-00')sobh
						WHERE 	ed <= NOW()
				)UNION ALL
				(
					SELECT * FROM (
					SELECT a.`kodecabang`,a.`kode_sobh`,a.`tgl_sobh`,b.namasupplier,a.`kode_produk`,
							...
							LEFT JOIN (SELECT a.`kodeproduk`,a.namaproduk,a.`kodesupplier`,b.`namasupplier`,a.satuan FROM `mstproduk` a 
										LEFT JOIN `mstsupplier2` b ON a.`kodesupplier`=b.`kodesupplier`) b ON a.`kode_produk`=b.kodeproduk 
							WHERE a.kode_sobh='$kode' AND a.ed_2 <> '0000-00-00')sobh
						WHERE 	ed <= NOW()
				)UNION ALL
				(
					SELECT * FROM (
					SELECT a.`kodecabang`,a.`kode_sobh`,a.`tgl_sobh`,b.namasupplier,a.`kode_produk`,
							...
							LEFT JOIN (SELECT a.`kodeproduk`,a.namaproduk,a.`kodesupplier`,b.`namasupplier`,a.satuan FROM `mstproduk` a 
										LEFT JOIN `mstsupplier2` b ON a.`kodesupplier`=b.`kodesupplier`) b ON a.`kode_produk`=b.kodeproduk 
							WHERE a.kode_sobh='$kode' AND a.ed_3 <> '0000-00-00')sobh
						WHERE 	ed <= NOW() ORDER BY ed ASC

				)
		)gabsobh ORDER BY namaproduk ASC, ed ASC";
		} 
 	...
 	function mendekatiEdTable(){
		...
		if(empty($kode)){
			...
			$query 	= "SELECT * FROM (	(	
					...
					)gabsobh ORDER BY namaproduk ASC, ed DESC";
		} 
...
//	$tgl = date('d M Y');
	...
	$pdf->Ln(2); 
    $pdf->Cell(0,5,'Kami Cabang '.$namacabang.' Telah Melaksanakan SOBH dengan Nomor '.$kode.' pada tanggal '.$tgl,0,1,'L');
...
//	$pdf->SetLeftMargin(2);
    ...
    $pdf->Cell(0,5,'Total Unit Selisih  = '.$totselisihunitsobhxx,0,1,'L');
    $pdf->Cell(0,5,'Total Value Selisih  = RP. '.$totselisihvaluesobhxx,0,1,'L');

	$pdf->SetFont('arial','B',8);
    $pdf->Cell(0,4,'Keterangan',0,1,'L');
    $pdf->Cell(0,4,'* TUnK		= Total Unit Komputer',0,1,'L');
    $pdf->Cell(0,4,'* TValK		= Total Value Komputer',0,1,'L');
    $pdf->Cell(0,4,'* TUnF		= Total Unit Fisik',0,1,'L');
    $pdf->Cell(0,4,'* TValF		= Total Value Fisik',0,1,'L');
    $pdf->Cell(0,4,'* TUnSel	= Total Unit Selisih',0,1,'L');
    $pdf->Cell(0,4,'* TValSel	= Total Value Selisih',0,1,'L');
	$pdf->Ln(5);
...
//	$pdf->Cell(50,5,'Nama Supplier',1,0,'C',true);
	...
	$pdf->SetLeftMargin(2);
	$pdf->SetFont('arial','',8);
    $pdf->Cell(0,5,'D. Data Produk sobh',0,1,'L');
	$pdf->Ln(2);
	$pdf->SetFont('arial','B',8);
	$pdf->SetLineWidth(.1);
	$pdf->SetFillColor(229,229,229);
	...
	$pdf->Cell(30,5,'*STOCK OPNAME BARANG HARIAN INI ADALAH BENAR BAHWA CABANG KAMI MELAKUKAN OPNAME DENGAN YANG TERTERA DIATAS.',0,1,'L',false);
	$pdf->Ln(1);
	$pdf->SetFont('arial','',7);	
	...
	$pdf->Output('Cetak_SOBH_'.$kode.'.pdf','D'); //D=Download, I= ,
  } else {
    echo "<h1>Maaf, data tidak ditemukan</h1><br>";
	echo $query."</br>";
  }
