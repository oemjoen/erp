  ...
  date_default_timezone_set('Asia/Jakarta'); 
  	...
  	$kode	= $_GET[kode];
   		...
   		if(empty($kode)){
			...
			$query = "SELECT * FROM pembelian_pre";
			echo "<h1>Maaf, data tidak ditemukan</h1><br>";
		}else{
			$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang FROM pembelian_pre a 
   ...
   if ($row>0) {
	...
	function Footer()
	{
	   ...
	   function FancyTable(){
			...
			$kode	= $_GET[kode];   		
				if(empty($kode)){
							$query = "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan FROM pembelian_pre a 
												LEFT JOIN mstproduk b ON a.`kode_barang`=b.`kodeproduk` 
												LEFT JOIN mastercabang c ON a.`cabang`=c.`KdCabang`";
					}else{
							$query 	= "SELECT a.*,b.`namaproduk`,c.NmCabang,b.kandungan,b.sediaan FROM pembelian_pre a 
...
//	$tgl = date('d M Y');
    ...
    $pdf->Cell(30,5,': Penanggung Jawab PT. Sapta Sari Tama Cabang '.cari_nama_cabang($kode),0,1,'L');
	...
	$pdf->Cell(73,3,'',0,0,'C',false);
	$pdf->Cell(73,3,cari_kota_pr($kode).", ".$tgl,0,1,'C',false);
	$pdf->Ln(2);
...
//	if(app_kepala_gud($kode)=='Y' && $namacabang=='PUSAT'){$pdf->Image('images/gudpusat.png',28,$pdf->GetY(),20);}	if(app_apt($kode)=='Y' && $namacabang=='PUSAT'){$pdf->Image('images/aptpusat.png',70,$pdf->GetY(),35);}	if(app_bm($kode)=='Y' && $namacabang=='PUSAT'){$pdf->Image('images/bmpusat.png',120,$pdf->GetY(),35);}
	...
	$pdf->Cell(73,3,'*'.tgl_apt($kode),0,1,'L',false);
