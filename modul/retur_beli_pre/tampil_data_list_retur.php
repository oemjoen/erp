...
if (empty($kode)){
	if ($cabang =='KPS')
	{
		$query2 = "SELECT DISTINCT a.kode_retur FROM retur_beli_pre a ORDER BY a.`tgl_retur` DESC,a.`cabang`,a.`kode_retur`";//echo "--1--".$query2;
	}
	else
	{
		$query2 = "SELECT DISTINCT a.kode_retur FROM retur_beli_pre a WHERE a.cabang='$cabang' ORDER BY a.`tgl_retur` DESC,a.`cabang`,a.`kode_retur`";//echo "--2--".$query2;
	}
}else{
	if ($cabang =='KPS')
	{
		$query2 = "SELECT * FROM retur_beli_pre WHERE kode_retur = '$kode'";//echo "--3--".$query2;
	}
	else
	{
		$query2 = "SELECT * FROM retur_beli_pre WHERE kode_retur = '$kode' AND cabang='$cabang'";//echo "--4--".$query2;
	}
}
			...
			if (empty($kode)){
				if ($cabang =='KPS'){
						$sql = "SELECT DISTINCT a.kode_retur,a.tgl_retur,a.kode_supplier,
									...
									LEFT JOIN (
											SELECT DISTINCT kodekonf_retur,kode_retur,tglkonf_retur,kode_prinsipal,`namaprinsipal` FROM konfirm_retur_beli_pre po
	...
	echo "<table width='100%'>
		...
		echo "<td align='right'>Halaman :";
			for ($h = 1; $h <= $max; $h++) {
                    ...
                    onClick="$.get('modul/retur_beli_pre/tampil_data_list_retur.php?cabang=<?php echo $cabang;?>&hal=<?php echo $list[$h]; ?>', 
	...
	echo "</td>";
		...
		echo "<td align='right'>Halaman :";
			for ($h = 1; $h <= $max; $h++) {
					$list[$h] = $lim * $h - $lim;
					echo " <a href='javascript:void(0)' ";?> 
                    onClick="$.get('modul/retur_beli_pre/tampil_data_list_retur.php?cabang=<?php echo $cabang;?>&kode=<?php echo $_GET['kode'];?>
