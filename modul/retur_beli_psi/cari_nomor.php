...
//$text	= "SELECT max(kode_beli) as no_akhir FROM retur_beli_psi where cabang='$cabang1'";
$text	= "SELECT MAX(kode_retur) AS no_akhir FROM retur_beli_psi WHERE cabang='$cabang1' AND CONCAT(MID(tgl_retur,6,2),LEFT(tgl_retur,4))='$countercek'";
...
if (empty($noerdit)){
		if ($row>0){
			$data=mysql_fetch_array($sql);	
				...
				//format kode beli RR/PSI/CAB/001/TGL/BLN/THN
				$no = $data['no_akhir'];
				...
				$kode_beli = 'RR/PSI/'.$cabang1.'/'.sprintf("%03s",$no_akhir).'/'.date(d).'/'.date(m).'/'.date(Y);
