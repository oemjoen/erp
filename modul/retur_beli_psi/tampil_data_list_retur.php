<script type="text/javascript">
	...
	function editRow(ID){
	   var kode = ID; 
		window.location.href='media.php?module=retur_beli_psi&no='+kode;

	}
	function cetakRow(ID){
	   var kode = ID; 
		window.location.href='modul/laporan/cetak_retur_psi.php?kode='+kode;
	...
	function lihatRow(ID){
		...
		var cabang	= $("#textcabang").val();
	   	...
	   	$.ajax({
			type	: "GET",
			url		: "modul/retur_beli_psi/tampil_data_list_retur.php",
...
if (empty($kode)){
	if ($cabang =='KPS')
	{
		$query2 = "SELECT DISTINCT a.kode_retur FROM retur_beli_psi a ORDER BY a.`tgl_retur` DESC,a.`cabang`,a.`kode_retur`";//echo "--1--".$query2;
	}
	else
	{
		$query2 = "SELECT DISTINCT a.kode_retur FROM retur_beli_psi a WHERE a.cabang='$cabang' ORDER BY a.`tgl_retur` DESC,a.`cabang`,a.`kode_retur`";//echo "--2--".$query2;
	}
}else{
	if ($cabang =='KPS')
	{
		$query2 = "SELECT * FROM retur_beli_psi WHERE kode_retur = '$kode'";//echo "--3--".$query2;
	}
	else
	{
		$query2 = "SELECT * FROM retur_beli_psi WHERE kode_retur = '$kode' AND cabang='$cabang'";//echo "--4--".$query2;
	}
}
			...
			if (empty($kode)){
				if ($cabang =='KPS'){
						$sql = "SELECT DISTINCT a.kode_retur,a.tgl_retur,a.kode_supplier,
									...
									FROM retur_beli_psi a
									LEFT JOIN mstsupplier2 b ON a.`kode_supplier`=b.`kodesupplier`
									LEFT JOIN (
											SELECT DISTINCT kodekonf_retur,kode_retur,tglkonf_retur,kode_prinsipal,`namaprinsipal` FROM konfirm_retur_beli_psi po
		...
		else
			{
				if ($cabang =='KPS'){
						$sql = "SELECT a.*,b.`namasupplier`,c.`namaproduk`,c.`satuan`,cc.`alasan`,dd.`asal_retur_ket`
							...
							LEFT JOIN `mst_asal_retur` dd ON a.`asal_retur`=dd.`kode_asal`							WHERE a.kode_retur='$kode'
						...
						$sql = "SELECT a.*,b.`namasupplier`,c.`namaproduk`,c.`satuan`,cc.`alasan`,dd.`asal_retur_ket`
							...
							LEFT JOIN `mstproduk` c ON a.`kode_barang`=c.`kodeproduk`
							LEFT JOIN mst_alasan_retur cc ON a.`alasan_retur`=cc.`kode_alasan`
							LEFT JOIN `mst_asal_retur` dd ON a.`asal_retur`=dd.`kode_asal`							WHERE a.kode_retur='$kode' AND cabang='$cabang'
							ORDER BY a.kode_retur DESC, a.kode_barang ASC
	...
	echo "<table width='100%'>
		...
		echo "<td align='right'>Halaman :";
			for ($h = 1; $h <= $max; $h++) {
                    ...
                    onClick="$.get('modul/retur_beli_psi/tampil_data_list_retur.php?cabang=<?php echo $cabang;?>&hal=<?php echo $list[$h]; ?>', 
	...
	echo "</td>";
		...
		echo "<td align='right'>Halaman :";
			for ($h = 1; $h <= $max; $h++) {
					$list[$h] = $lim * $h - $lim;
					echo " <a href='javascript:void(0)' ";?> 
                    onClick="$.get('modul/retur_beli_psi/tampil_data_list_retur.php?cabang=<?php echo $cabang;?>&kode=<?php echo $_GET['kode'];?>
