...
$query	= "SELECT CONCAT(kode_retur,kode_supplier,kode_barang,batch) AS kode 
	FROM retur_beli_psi WHERE CONCAT(kode_retur,kode_supplier,kode_barang,batch)= '$id'";
$sql 	= mysql_query($query);
$row	= mysql_num_rows($sql);
if ($row>0){
	$input = "DELETE FROM retur_beli_psi WHERE CONCAT(kode_retur,kode_supplier,kode_barang,batch)= '$id'";
	mysql_query($input);
