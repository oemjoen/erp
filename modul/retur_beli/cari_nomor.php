...
//$text	= "SELECT max(kode_beli) as no_akhir FROM retur_beli where cabang='$cabang1'";
$text	= "SELECT MAX(kode_retur) AS no_akhir FROM retur_beli WHERE cabang='$cabang1' AND CONCAT(MID(tgl_retur,6,2),LEFT(tgl_retur,4))='$countercek'";
...
if (empty($noerdit)){
		if ($row>0){
			$data=mysql_fetch_array($sql);	
				...
				//format kode beli RR/CAB/001/TGL/BLN/THN
				$no = $data['no_akhir'];
				$no_akhir = (int) substr($no, 8, 3);
				...
				$kode_beli = 'RR/'.$cabang1.'/'.sprintf("%03s",$no_akhir).'/'.date(d).'/'.date(m).'/'.date(Y);
