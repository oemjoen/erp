...
$cektgl2 = $mth1.$yr1;
	...
	echo 
		"<p align='left'><font size='3'><b>CEK STOK CABANG<br>
...
<div id='info'>
	...
	$sql1	= "SELECT Cabang,b.Prinsipal,a.Produk,b.Produk AS `NamaProduk`,
					...
					SELECT Cabang,Produk,0 AS awal,
						SUM(CASE WHEN `Status BPB` IN ('Open','DO','BPB Retur','BPB Relokasi','BPB Koreksi','BPB Konversi') THEN ((IFNULL(`Qty Terima`,0) + IFNULL(`Bonus`,0))) ELSE 0 END) AS masuk,
						SUM(CASE WHEN `Status BPB` IN ('BKB Retur','BKB Relokasi','BKB Koreksi','BKB Konversi') THEN ABS((IFNULL(`Qty Terima`,0) + IFNULL(`Bonus`,0)))ELSE 0 END) AS keluar,
						0 AS akhir
					FROM `dbpbdodetail`
	...
	$sql1	= "SELECT Cabang,b.Prinsipal,a.Produk,b.Produk AS `NamaProduk`,
					...
					LEFT JOIN (SELECT `Prinsipal`,`Pabrik`,`Kode Produk`,`Produk` FROM `mproduk`) AS b ON a.Produk=b.`Kode Produk` 
					GROUP BY Cabang,Produk
					HAVING (Awal + Masuk - Keluar != Akhir)
					ORDER BY Cabang,b.Prinsipal,a.Produk
					";		
	}else{
	$sql1	= "SELECT Cabang,b.Prinsipal,a.Produk,b.Produk AS `NamaProduk`,
		...
		{
			$no++;
			if($r_data[awal] + $r_data[masuk] - $r_data[keluar]==$r_data[akhir])
			{
			echo"	<tr>
			...
			echo"	<tr>
						...
						<td align='left' bgcolor='red'>$r_data[Cabang]</td>
						<td align='left' bgcolor='red'>$r_data[Prinsipal]</td>
						<td align='left' bgcolor='red'>$r_data[Produk]</td>
						<td align='left' bgcolor='red'>$r_data[NamaProduk]</td>
						<td align='right' bgcolor='red'>".format_rupiah_koma($r_data[awal])."</td>
						<td align='right' bgcolor='red'>".format_rupiah_koma($r_data[masuk])."</td>
						<td align='right' bgcolor='red'>".format_rupiah_koma($r_data[keluar])."</td>
						<td align='right' bgcolor='red'>".format_rupiah_koma($r_data[akhir])."</td>
					</tr>";
			}
